...
// A camera class that processes input and calculates the corresponding Eular Angles, Vectors and
// Matrices for use in OpenGL Constructor with vectors
EulerCamera::EulerCamera( glm::vec3 position, glm::vec3 up, GLfloat yaw, GLfloat pitch ) :
    Camera( position, up, glm::vec3( 0.0f, 0.0f, -1.0f ), ZOOM ),
...
// direction.
void EulerCamera::processmousemovement( int button,
                                        GLfloat xpos,
                                        GLfloat ypos,
...
void EulerCamera::updatecameravectors() {
    ...
    _right = glm::normalize( glm::cross(
        _front, _worldup ) ); // Normalize the vectors, because their length gets closer to 0 the
...
void TrackballCamera::processmousemovement( int button,
                                            GLfloat xpos,
                                            GLfloat ypos,
                                            GLboolean constrainPitch ) {
    (void)button;
...
glm::vec3 TrackballCamera::getmouseprojectiononball( float xpos, float ypos ) {
    glm::vec3 mouseonball = glm::vec3( ( xpos - _viewport.z * 0.5f ) / ( _viewport.z * 0.5f ),
...
glm::vec2 TrackballCamera::getmouseonscreen( float xpos, float ypos ) {
    return glm::vec2( ( xpos - _viewport.z * 0.5f ) / ( _viewport.z * 0.5f ),
