...
enum Camera_Movement { LEFT = 0, FORWARD, RIGHT, BACKWARD };
...
class Camera
{
  ...
  public:
    ...
    // and y direction.
    virtual void
    processmousemovement( int button, GLfloat xpos, GLfloat ypos, GLboolean constraint = true );
...
class EulerCamera : public Camera
{
  ...
  public:
    ...
    EulerCamera( glm::vec3 position = glm::vec3( 0.0f, 0.0f, 0.0f ),
    ...
    // Processes input received from a mouse input system. Expects the offset value in both the x
    // and y direction.
    void processmousemovement( int button,
                               GLfloat xpos,
                               GLfloat ypos,
