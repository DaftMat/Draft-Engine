...
Engine::Engine( int width, int height ) :
    ...
    m_shaderselector.emplace_back( []() -> Shader* {
        return new Shader( "shaders/phong.vert.glsl", "shaders/phong.frag.glsl" );
    } );
    m_shaderselector.emplace_back( []() -> Shader* {
        return new Shader( "shaders/blinnphong.vert.glsl", "shaders/blinnphong.frag.glsl" );
    } );
    m_shaderselector.emplace_back( []() -> Shader* {
        return new Shader( "shaders/pbr.vert.glsl", "shaders/pbr.frag.glsl" );
    } );
    m_shaderselector.emplace_back( []() -> Shader* {
        return new Shader( "shaders/error.vert.glsl", "shaders/error.frag.glsl" );
    } );
    m_shaderselector.emplace_back( []() -> Shader* {
        return new Shader( "shaders/default.vert.glsl", "shaders/default.frag.glsl" );
    } );
    m_shader.reset( m_shaderselector[m_activeshader]() );

    /// Setup cameras
    m_cameraselector.emplace_back(
        []() -> Camera* { return new EulerCamera( glm::vec3( 0.f, 0.f, 6.f ) ); } );
    m_cameraselector.emplace_back( []() -> Camera* {
        return new TrackballCamera(
            glm::vec3( 0.f, 0.f, 6.f ), glm::vec3( 0.f, 1.f, 0.f ), glm::vec3( 0.f, 0.f, 0.f ) );
    } );
...
void Engine::mouseclick( int button, float xpos, float ypos ) {
    ...
    m_mousex         = xpos;
    m_mousey         = ypos;
    m_clicked_object = m_modelmanager->mouse_click(
        Ray( xpos, m_height - ypos, m_width, m_height, m_projection, m_camera->viewmatrix() ),
        xpos,
        m_height - ypos );
...
void Engine::shaderChanged( GLuint selected ) {
    ...
    if ( m_activeshader >= m_shaderselector.size() ) m_activeshader = m_shaderselector.size() - 1;
    m_shader.reset( m_shaderselector[m_activeshader]() );
}
