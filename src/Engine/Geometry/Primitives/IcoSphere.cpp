...
void IcoSphere::setupIcosahedron() {
    ...
    for ( int i = 1; i <= 5; ++i )
    {
        ...
        vertex.Position = vertex.Normal =
            glm::vec3( xy * glm::cos( h_angle1 ), z, xy * glm::sin( h_angle1 ) );
        vertices.push_back( vertex );

        vertex.Position = vertex.Normal =
            glm::vec3( xy * glm::cos( h_angle2 ), -z, xy * glm::sin( h_angle2 ) );
        vertices.push_back( vertex );
...
void IcoSphere::setupSubdivisions() {
    ...
    for ( GLuint i = 0; i < tmpIndices.size(); i += 3 )
    {
        ...
        /// 3 new vertices
        new_v1.Position = new_v1.Normal =
            computeHalfVertex( tmpVertices[v1_ind].Position, tmpVertices[v2_ind].Position );
        new_v2.Position = new_v2.Normal =
            computeHalfVertex( tmpVertices[v2_ind].Position, tmpVertices[v3_ind].Position );
        new_v3.Position = new_v3.Normal =
            computeHalfVertex( tmpVertices[v1_ind].Position, tmpVertices[v3_ind].Position );
